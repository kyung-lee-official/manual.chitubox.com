...
const absRootPath = path.resolve("../pages");
const rootItems = await readdir(absRootPath);
const absLocalePaths: string[] = [];
for (const item of rootItems) {
	const absItemPath = path.resolve(absRootPath, item);
	const absRootPathStat = await lstat(absItemPath);
	if (item !== "api" && absRootPathStat.isDirectory()) {
		/* item is a locale directory */
		try {
			/* Check whether there is a "docs" directory */
			await access(path.resolve(absItemPath, "docs"));
			const docsStat = await lstat(path.resolve(absItemPath, "docs"));
 ...
 */
function getDocInstanceContext(localizedDocInstanceContexts: any, targetDocInstance: string) {
 ...
 */
function getVersionedContext(docInstanceContext: any, targetVersionCode: string) {
