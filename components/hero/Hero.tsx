...
const Hero = () => {
	...
	const [dangleOpacity, setDangleOpacity] = useState<number>(0);
	const [repeatDep, setRepeatDep] = useState<boolean>(true);
	...
	useEffect(() => {
		...
		const dangle = document.getElementById("dangle") as HTMLElement;
		const sequence = async () => {
			await animate(
				dangle,
				{ d: [dropShapes[0].d, dropShapes[1].d] } as any,
				{ duration: 6 }
			);
			setDangleOpacity(1);
			setDropOpacity(0);
			await animate(
				drop,
				{ d: [waterDrop[0].d, waterDrop[1].d] } as any,
				{
					ease: "easeInOut",
					duration: 0.2,
				}
			...
			setDangleOpacity(0);
			await animate(dangle, {
				d: [dropShapes[1].d, dropShapes[0].d],
			} as any);
			setDropOpacity(1);
			setRepeatDep(!repeatDep);
		};
