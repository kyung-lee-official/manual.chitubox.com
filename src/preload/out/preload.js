...
for (const absLocalePath of absLocalePaths) {
    ...
    const docInstances = itemsInLocaleFolder.filter(async (item) => {
        const absLocalePathState = await lstat(path.resolve(absLocalePath, item));
    ...
    const orderedDocInstanceIdList = JSON.parse(await readFile(path.resolve(absLocalePath, "config.json"), "utf-8"));
    ...
    for (const docInstance of orderedDocInstances) {
        ...
        if (docInstanceStat.isDirectory()) {
            const config = JSON.parse(await readFile(path.resolve(absDocInstancePath, "config.json"), "utf-8"));
            const docInstanceContext = getDocInstanceContext(localizedContext.localizedDocInstances, docInstance);
                    ...
                    if (item !== "config.json") {
                        ...
                        for (const pageContext of versionedContext.pagesContext) {
                            ...
                            if (pageContext.subItems) {
                                ...
                                absPagePath = path.resolve(absVersionedPath, pageContext.item, "page.mdx");
                                ...
                                for (const subPageContext of pageContext.subItems) {
                                    urlPath = `/${absVersionedPath
                                        .split(path.sep)
                                        .slice(systemPathLength)
                                        .join(path.posix.sep)}/${pageContext.item}/${subPageContext.item}`;
                                    subPageContext.path = urlPath;
                                    absPagePath = path.resolve(absVersionedPath, pageContext.item, subPageContext.item, "page.mdx");
                                    const toc = await getTocContext(absPagePath);
 ...
 * @returns localized context
 */
function getLocalizedContext(docsContext, targetUrlLocale) {
 ...
 */
function getDocInstanceContext(localizedDocInstanceContexts, targetDocInstance) {
    return localizedDocInstanceContexts.find((docInstance) => {
 ...
 */
function getVersionedContext(docInstanceContext, targetVersionCode) {
    return docInstanceContext.find((versionedContext) => {
        return versionedContext.versionCode === targetVersionCode;
    });
}
function getPageContext(versionedContext, item) {
    return versionedContext.find((pageContext) => {
        return pageContext.item === item;
    });
}
async function getTocContext(absPagePath) {
...
async function recurseVersionedPath(absPath, pagesContext) {
