...
The "Open Log Path" option in CHITUBOX allows you to access the folder containing the log file.

In case the software crashes and cannot be opened, you can access its path directly.

* Windows: `C:\Users\{user-name}\AppData\Local`

* macOS: `Macintosh HD/Users/{user-name}/Library/Application Support`

* Ubuntu: `~/.local/share/chitubox2_0`
...
The "Settings" option in CHITUBOX allows you to modify the settings for the software. By clicking the "Settings" button, you will be able to open a window containing the Appearance, File, System, and Function tabs. This is useful for customizing the settings to fit your needs.
...
This allows you to change the pattern of the sectional area.
...
Set the naming rules for the sliced file. Most rules are self-explanatory, so we will only explain the ones that needs further explanation.
...
Some operating systems do not allow certain characters in filenames. This allows you to replace those characters with other characters.

#### Custom Character Replacement

You can add your own character replacement rules here.
...
This allows you to change the rendering interface of the software.
...
This option allows you to select a model that has slight differences from the model with supports added and replace the current model without having to add supports again.

### Select Model

If the cursor is projected on a model that is not selected, this option will select the model. You can also hold down the Ctrl key and click the model to select multiple models.

### Cancel Seleted Model

If the cursor is projected on a model that is already selected, this option will cancel the selection of the model.

### Hide

Hide the model that the cursor is projected on.

### Hide Others

Hide all models except the model that the cursor is projected on.
