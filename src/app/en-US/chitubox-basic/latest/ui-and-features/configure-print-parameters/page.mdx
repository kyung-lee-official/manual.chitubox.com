...
<Admonition type="tip">
	Migrating from Anycubic Photon Workshop? Heads up! The order of TSMC parameters in the UI is different.

	In Photon Workshop, [0] refers to the motion stage closer to the screen, while [1] represents the stage farther away. CHITUBOX, however, follows the motion order when laying out the input text boxes. This can be confusing at first.
</Admonition>
