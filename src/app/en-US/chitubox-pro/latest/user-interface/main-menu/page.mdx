...
Click the menum button <ChituboxMenu size={24} /> on the top left corner to open the main menu.
...
Save all or seleted models to a _.chitubox_ project file.
...
Export all or seleted models (including supports).
...
You can subit any ideas, suggestions and foodback about CHITUBOX to our development team.
...
Check if the current software version needs updates and dispaly update logs for different versions.
...
Export current machine settings to a _.cfg_ file. This function helps to backup print configurations or share your configuration with other users.

### Import Configuration

Choose a _.cfg_ file and import to the current machine settings. This function helps to restore print settings or use print settings shared by other users.
...
Manage software plugins. You can import or export plugins to support printers that do not use Chitu Systems. 
...
When enabled, the saving diretory will always be the same as the lastest directory for opening files.
...
Set rules for naming slice files. Most rules are self-explanatory. Only a few special rules are explained below.
...
#### Support Auxillary Line

Enable/disable support auxillary line.
...
Enable/disable print estimation pannel at the bottome left corner.

#### Allow Raft Overlap in Collision Detection

When enabled, collition detection ignores support raft overlaps and will not generate warnings for them.
