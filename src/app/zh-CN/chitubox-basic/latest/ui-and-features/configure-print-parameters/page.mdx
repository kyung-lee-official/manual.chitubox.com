...
在配置参数面板，你可以 <ChituboxProfileDownloadIcon /> **下载配置参数** (材料库或自定义), <ChituboxProfileUploadIcon /> **上传所有配置参数** (自定义), <ChituboxProfileAddIcon /> **添加配置参数**, <ChituboxProfileImportIcon /> **导入配置参数** (本地), <ChituboxProfileExportAllIcon /> **导出所有配置参数** (本地).

将鼠标悬浮在配置参数的图标 <ChituboxProfileMenuIcon /> 上将弹出对于该配置的可用操作, 包括 <ChituboxProfileUploadIcon /> 上传, <ChituboxProfileRenameIcon /> 重命名, <ChituboxProfileExportIcon /> 导出, <ChituboxProfileResetIcon /> 重置, <ChituboxProfileDeleteIcon /> 删除。
