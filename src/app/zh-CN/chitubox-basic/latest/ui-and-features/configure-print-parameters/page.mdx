...
<Admonition type="note" title="注意">
	并非所有打印机都支持**静止时间**等待模式。在这里查看<Link href="https://wkhvloo2kw.feishu.cn/docx/W9XedDtuqoHK2IxsicvccQthngf">📃完整的兼容性列表</Link>。
</Admonition>
...
底层光强的 <Link href="https://en.wikipedia.org/wiki/Pulse-width_modulation">PWM</Link>，范围从 `0` 到 `255` 或从 `0%` 到 `100%`，控制底层的 UV 光源功率。

### 光强 PWM

普通层光强的 <Link href="https://en.wikipedia.org/wiki/Pulse-width_modulation">PWM</Link>，范围从 `0` 到 `255` 或从 `0%` 到 `100%`，控制普通层的 UV 光源功率。
